...
module.exports = {
    ...
    run: async (client, message, args, prefix, color) => {
        ...
        if (client.config.owner.includes(message.author.id) || db.get(`ownermd_${client.user.id}_${message.author.id}`) === true) {
            ...
            if (args[0] === "add") {
                let ss = message.mentions.channels.first() || 
                         message.guild.channels.cache.get(args[1]) || 
                         message.guild.channels.cache.find(c => c.name === args.slice(1).join(' '));

                if (ss) {
                    if (db.get(`channelpublic_${message.guild.id}_${ss.id}`) === true) {
                        return message.channel.send(`Les commandes publiques dans ${ss} sont déjà activées`);
                    }
                    ...
                    return message.channel.send(":x: Salon introuvable. Assurez-vous d'utiliser une mention, un ID valide ou un nom de salon existant.");
                }

            } else if (args[0] === "remove") {
                let ss = message.mentions.channels.first() || message.guild.channels.cache.get(args[1]);
                if (ss) {
                    if (db.get(`channelpublic_${message.guild.id}_${ss.id}`) === null) {
                        return message.channel.send(`Les commandes publiques dans ${ss} sont déjà désactivées`);
                    }
                ...
                message.channel.send(`${money.length === undefined || null ? 0 : money.length} ${money.length > 1 ? "channels ont été supprimées " : "channel a été supprimé"} des public channel`);
